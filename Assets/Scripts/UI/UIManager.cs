...
public class UIManager : MonoBehaviour
{
    ...
    private void EnsureUIElements()
    {
        ...
        if (turnText == null)
        {
            turnText = CreateText("TurnText", cachedCanvas.gameObject, mobile ? new Vector2(-450, 110) : new Vector2(-300, 140), "Turn: 1 - Player1", mobile ? 30 : 20);
            if (mobile)
            {
                turnText.fontStyle = FontStyle.Bold;
                var rt = turnText.GetComponent<RectTransform>();
                rt.anchoredPosition = new Vector2(-450, 110);
                rt.sizeDelta = new Vector2(400, 35);
            }
        }
        if (movesRemainingText == null)
        {
            movesRemainingText = CreateText("MovesText", cachedCanvas.gameObject, mobile ? new Vector2(-450, 95) : new Vector2(-300, 125), "Moves left: 0", mobile ? 27 : 18);
            if (mobile)
            {
                movesRemainingText.fontStyle = FontStyle.Bold;
                var rt = movesRemainingText.GetComponent<RectTransform>();
                rt.anchoredPosition = new Vector2(-450, 95);
                rt.sizeDelta = new Vector2(400, 35);
            }
        }
        if (phaseText == null)
        {
            ...
            if (mobile)
            {
                ...
                rt.anchoredPosition = new Vector2(-450, 80);
                rt.sizeDelta = new Vector2(400, 35);
            }
        }
        if (timerText == null)
        {
            ...
            if (mobile)
            {
                ...
                rt.anchoredPosition = new Vector2(-100, 105);
